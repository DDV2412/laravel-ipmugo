<x-guest-layout>
    <x-jet-authentication-card>
        ...
        @if (session('status'))
            <div class="mb-4 font-medium text-sm text-blue-500">
                {{ session('status') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <h1 class="text-xl font-extrabold mt-2">{{ __('Login to your accounts') }}</h1>
            <p class="text-sm py-1">{{ __('Dont have account?') }} <a href="{{ route('register') }}" class="text-blue-500 font-semibold">{{ __('Sign up here') }}</a></p>
