...
class Articles extends Component
{
    ...
    {   $count = ArticleRecord::count();
        ...
        if($this->repositoryId)
        {
            if(!empty($this->years))
            {
                $articles = ArticleRecord::query()->search($this->search)->where(['year' => $this->years, 'repoId' => $this->repositoryId])->orderBy($this->sortBy, $this->sortDirection)->paginate($this->paginate);
            }else{
                $articles = ArticleRecord::query()->search($this->search)->where(['repoId' => $this->repositoryId])->orderBy($this->sortBy, $this->sortDirection)->paginate($this->paginate);
            }
        }elseif($this->years)
        {
            $articles = ArticleRecord::query()->search($this->search)->where(['year' => $this->years])->orderBy($this->sortBy, $this->sortDirection)->paginate($this->paginate);
        }
        else{
            $articles = ArticleRecord::query()->search($this->search)->orderBy($this->sortBy, $this->sortDirection)->paginate($this->paginate);
        }

        $repositories = Repository::all();

        
        return view('livewire.frontend.articles', compact('count','repositories','articles', 'lastYear', 'firstYear'))
        ->extends('layouts.app')
        ->section('content');
    }
